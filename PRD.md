...
Rapidly assess new content, devise a plan, new agents get things done.

Pillars 4: guardrails, observability, tools, harness.

Harness; car; multiturn looping, validate inputs outputs, guardrails, observability.

Model; engine.

The following criteria matter when building:

- Ability to repeat, maintain history, add information in loop.
- Tools -- function to add info to LLM, schema, executor.
- Guardrail (keep it in bounds) -- input, action, output.
- Observability -- p95 latency, $/run, error %, eval.

Agent will be able to fill out the 1040 with a sample W-2 and get the actual and correct results as if filing with the government.

This will be deployed on Render.

W-2s to upload will be in PDF format.

Please look at `1040.pdf` and look into how and what needs to be filled out, calculated, and how. Also look at `samplew2.pdf` to see what a sample W-2 form would look like, and the information needed to fill out a 1040 form.

Reference the proactive and on-demand agent in `C:\Users\jaynyasg\OneDrive\Documents\GitLab\ac` for agent build. Please be aware of the AI costs and analysis and guidelines.

Reference `C:\Users\jaynyasg\OneDrive\Documents\GitLab\Taxes` for other tax filing repos to see how they get their information and build their agents. Our agent will need tax knowledge, tax filing knowledge, and 1040 knowledge.

Reference `C:\Users\jaynyasg\OneDrive\Documents\GitLab\house-style` for design.
